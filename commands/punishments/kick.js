module.exports = {
    ...
    execute(client, connection, config, message, args) {
        ...
        connection.query(`INSERT INTO Punishments (type, reason, punishmentTo, punishmentFrom) VALUES ("Kicked", "${reason}", ${user.id}, ${message.author.id})`, (err, result) => {
            if (err) return client.helpers.get('MYSQLErrorHandler').InsertError(message.channel, err, client);
            let kickedEmbed = client.helpers.get('CreateEmptyEmbed').execute("Warning", client, false)
            .setTitle(`${user.username} kicked`)
            .setThumbnail(user.avatarURL)
            .setDescription(`${message.author.username} kicked ${user.username} for \`\`${reason}\`\``);
            ...
            client.channels.find(channel => channel.id == config.logging.punishmentChannel).send(kickedEmbed);
            user.send(client.helpers.get('CreateEmptyEmbed').execute("Warning", client, false)
                    .setTitle(`You've been kicked`)
                    .setThumbnail(message.author.avatarURL)
                    .setDescription(`You've been kicked for \`\`${reason}\`\` by ${message.author.username}`))
                .then(m => {
